...
//    val accessToken: StateFlow<String> = _accessToken
    ...
    private val _isPreview = MutableStateFlow<UserPreviewModel?>(null)
    val isPreview: StateFlow<UserPreviewModel?> = _isPreview
...
//    }
    ...
    fun getPreview(accessToken: String) {
        viewModelScope.launch {
            try {
                _isLoading.value = true
                Log.i("dev-debug", "Token: $accessToken")
                val response =
                    apiService.getPreview("Bearer $accessToken")
                _isPreview.value = response.data
                Log.i("dev-debug", "Perfil obtenido con éxito")
            } catch (e: HttpException) {
                val messageError = evaluateHttpException(e)
                Log.e("dev-debug", "Error al obtener el perfil: $messageError")
